...
async function testScan() {
    const repoUrl = 'https://github.com/vulnerable-repo/test-repo'; // Placeholder
    console.log(`[Test] Triggering scan for ${repoUrl}...`);

    try {
        const postRes = await axios.post(`${API_URL}/scan`, { repo_url: repoUrl });
        ...
        while (!completed && attempts < 30) {
            const getRes = await axios.get(`${API_URL}/scan/${scan_id}`);

...
export const getRemediationFix = async (vulnerabilityId: string) => {
    try {
        ...
        const existingFixes = await databases.listDocuments(
            DB_ID,
            'vulnerability_fixes',
            [Query.equal('vulnerability_id', vulnerabilityId), Query.limit(1)]
        );
        ...
        let fileContent = '';
        if (repo.local_path && vuln.file_path) {
            try {
        ...
        // 6. Store and Return
        const savedFix = await databases.createDocument(
            DB_ID,
            'vulnerability_fixes',
            ID.unique(),
            {
        ...
        console.error('[AI Service] remediation fix generation failed:', err);
        throw err;
    }
};
...
export const recordFeedback = async (fixId: string, feedback: any) => {
    try {
        ...
        console.error('[AI Service] Error recording feedback:', error);
        throw error;
    }
};

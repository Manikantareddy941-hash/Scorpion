    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: '16px', width: '520px', maxWidth: '95vw', padding: '32px', position: 'relative' }}>
        ...
        {tab === 'github' && (
          <div>
            ...
            <input
              ...
              style={{ width: '100%', background: 'var(--bg-primary)', border: '1px solid var(--border-subtle)', borderRadius: '8px', padding: '12px', color: 'var(--text-primary)', fontSize: '0.9rem', outline: 'none', boxSizing: 'border-box' }}
            />
            <p style={{ color: 'var(--text-secondary)', opacity: 0.6, fontSize: '0.75rem', marginTop: '8px' }}>Supports public and private repositories</p>
          </div>
        )}
        ...
        <button
          ...
          style={{ width: '100%', marginTop: '24px', padding: '14px', background: 'var(--accent-primary)', border: 'none', borderRadius: '8px', color: 'white', fontWeight: 800, fontSize: '0.9rem', letterSpacing: '0.15em', cursor: 'pointer', opacity: (tab === 'github' ? !repoUrl.trim() : files.length === 0) ? 0.4 : 1, transition: 'opacity 0.2s' }}>
          ⚡ INITIATE SCAN
        </button>
      </div>
    </div>

...
export default function Reports() {
  ...
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'infrastructure' | 'security' | 'ai' | 'compliance'>(
    'security',
  );
  ...
  const headerDetails = {
    infrastructure: {
      ...
      desc: 'System & Architecture Reports',
      icon: <Activity size={32} />,
    },
    security: {
      title: 'Security Audits',
      desc: 'Enterprise-Grade Evidence Generation',
      icon: <Shield size={32} />,
    },
    ai: {
      title: 'AI Security Briefing',
      desc: 'Automated Posture Assessment',
      icon: <Zap size={32} />,
    },
    compliance: {
      title: 'Compliance Audit',
      desc: 'Regulatory & Standard Checks',
      icon: <Scale size={32} />,
    },
  };
  ...
  const auditSpecsMap = {
    infrastructure: {
      ...
      footer:
        'Infrastructure maps are generated from active network scans and cloud provider APIs.',
      items: [
        {
          icon: <Activity size={16} className="text-blue-500" />,
          title: 'Network Architecture',
          desc: 'Detailed mapping of VPCs, subnets, and routing tables.',
        },
        {
          icon: <Cpu size={16} className="text-emerald-500" />,
          title: 'Compute Resources',
          desc: 'Analysis of EC2, Lambda, and container orchestration.',
        },
        {
          icon: <Database size={16} className="text-purple-500" />,
          title: 'Data Persistence',
          desc: 'Storage configurations, backups, and encryption status.',
        },
      ],
    },
    security: {
      ...
      items: [
        {
          ...
          desc: 'Overview of critical, high, and medium risk factors.',
        },
        {
          icon: <AlertTriangle size={16} className="text-orange-500" />,
          title: 'Vulnerability Matrix',
          desc: 'Detailed mapping of every detected finding with file paths.',
        },
        {
          icon: <CheckCircle2 size={16} className="text-blue-500" />,
          title: 'Remediation Path',
          desc: 'Recommended upgrade paths and CVE identifiers.',
        },
      ],
    },
    ai: {
      ...
      footer:
        'AI summaries are generated dynamically via large language models assessing the security state.',
      items: [
        {
          icon: <Zap size={16} className="text-yellow-500" />,
          title: 'Threat Modeling',
          desc: 'Predictive analysis of attack vectors and surface area.',
        },
        {
          icon: <Cpu size={16} className="text-indigo-500" />,
          title: 'Anomaly Detection',
          desc: 'Pattern recognition for unusual commit or build behaviors.',
        },
        {
          icon: <FileText size={16} className="text-rose-500" />,
          title: 'Executive Summary',
          desc: 'High-level risk communication tailored for stakeholders.',
        },
      ],
    },
    compliance: {
      ...
      items: [
        {
          ...
          desc: 'Direct alignment of controls to regulatory requirements.',
        },
        {
          icon: <CheckCircle2 size={16} className="text-emerald-500" />,
          title: 'Policy Enforcement',
          desc: 'Status of mandatory security policies and guardrails.',
        },
        {
          icon: <FileDown size={16} className="text-cyan-500" />,
          title: 'Auditor Evidence',
          desc: 'Pre-formatted artifacts ready for external compliance audits.',
        },
      ],
    },
  };
              ...
              <div className="space-y-4">
                {recentReports.length > 0 ? (
                  recentReports.map((report) => (
                    <div
                      key={report.$id}
                      className="flex items-center justify-between p-4 bg-[var(--bg-primary)] border border-[var(--border-subtle)] rounded-2xl group hover:border-[var(--accent-primary)]/40 transition-all"
                    >
                  ...
                  <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase italic text-center py-4">
              ...
              <div className="premium-card p-10 min-h-[500px] flex flex-col">
                <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-sm font-black uppercase italic text-[var(--text-primary)]">
                        AI Security Briefing
                      </h3>
                      <p className="text-[8px] font-bold text-[var(--text-secondary)] uppercase tracking-[0.2em]">
                ...
                {isAiLoading ? (
                  <div className="flex-1 flex flex-col items-center justify-center space-y-4">
                    ...
                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest italic animate-pulse">
                      Echo is analyzing your security mesh...
                    </p>
                  </div>
                ) : aiSummary ? (
                  <div className="prose prose-invert prose-sm max-w-none">
                    <ReactMarkdown
                      components={{
                        h1: ({ node, ...props }) => (
                          <h1
                            className="text-xl font-black uppercase italic text-indigo-400 mt-6 mb-4"
                            {...props}
                          />
                        ),
                        h2: ({ node, ...props }) => (
                          <h2
                            className="text-lg font-black uppercase italic text-indigo-300 mt-5 mb-3"
                            {...props}
                          />
                        ),
                        h3: ({ node, ...props }) => (
                          <h3
                            className="text-md font-black uppercase italic text-indigo-200 mt-4 mb-2"
                            {...props}
                          />
                        ),
                        p: ({ node, ...props }) => (
                          <p
                            className="text-zinc-400 leading-relaxed mb-4 text-[11px] font-medium"
                            {...props}
                          />
                        ),
                        ul: ({ node, ...props }) => (
                        ...
                        li: ({ node, ...props }) => (
                          <li className="text-zinc-300 text-[11px]" {...props} />
                        ),
                        strong: ({ node, ...props }) => (
                          <strong className="text-indigo-200 font-black uppercase" {...props} />
                        ),
                      }}
                    >
                      {aiSummary}
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
                    ...
                    <p className="text-xs font-bold text-zinc-500 uppercase italic">
                      No AI Briefing available for this period. <br /> Click refresh to generate.
                    </p>
                  </div>
                )}
              </div>

...
import { ArrowLeft, Activity, Box, Download } from 'lucide-react';
            ...
            <tbody className="divide-y divide-[var(--border-subtle)]">
              {components.map((dep, i) => (
                <tr key={i} className="hover:bg-[var(--bg-primary)] transition-colors">
                  ...
                  <td className="px-6 py-4 text-xs text-[var(--text-secondary)] font-mono">{dep.fixVersion || '—'}</td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-0.5 rounded bg-[var(--bg-primary)] border border-[var(--border-subtle)] text-[9px] font-bold text-[var(--text-secondary)] uppercase">{dep.highestSeverity || 'NONE'}</span>
                  </td>
                </tr>
              ))}
            </tbody>
